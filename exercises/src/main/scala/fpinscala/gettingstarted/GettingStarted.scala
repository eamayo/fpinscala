...
object MyModule {
  ...
  def fib(n: Int): Int = {
    @annotation.tailrec
    def loop(index: Int, fibMinus1: Int, fibMinus2: Int): Int =
      if (n <= 0) 0
      else if (n == 1) 1
      else if (index == n) fibMinus1 + fibMinus2
      else loop(index+1, fibMinus1 + fibMinus2, fibMinus1)
...
object TestFib {
  ...
  def main(args: Array[String]): Unit = {
    println("Expected: 0, 1, 1, 2, 3, 5, 8, 13, 21")
    println("Actual:   %d, %d, %d, %d, %d, %d, %d, %d, %d".
        format(fib(0), fib(1), fib(2), fib(3), fib(4), fib(5), fib(6), fib(7), fib(8)))
  }
}
